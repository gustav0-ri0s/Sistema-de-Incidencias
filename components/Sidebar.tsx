...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed md:static inset-y-0 left-0 w-72 bg-white border-r border-gray-100 flex flex-col z-50 h-full transition-transform duration-300 ease-in-out transform ${isOpen ? 'translate-x-0' : '-translate-x-full'
          } md:translate-x-0`}
      >
        <div className="p-6 flex flex-col items-center border-b border-gray-50 relative shrink-0">
          <button
            onClick={onClose}
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto custom-scrollbar">
          {navItems.map((item) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center space-x-4 px-4 py-3 rounded-2xl font-bold transition-all ${isActive
        ...
        <div className="p-4 border-t border-gray-50 space-y-1 shrink-0">
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-3 text-slate-400 hover:text-brand-celeste rounded-2xl transition-all hover:bg-slate-50 border border-transparent hover:border-slate-100 group"
          >
          ...
          <button
            onClick={logout}
            className="flex items-center space-x-4 px-4 py-3 w-full rounded-2xl font-bold text-gray-400 hover:text-red-500 hover:bg-red-50 transition-all border border-transparent"
          >
