...
async function inspectTable() {
    ...
    if (error) {
        ...
        // Let's try to trigger a "column not found" error to see what columns exist
        const { error: error2 } = await supabase.from('students').select('not_a_column').limit(1);
        console.log('Schema error hint:', error2?.message);
    }
}
